...
import { Send, Hash, Settings, LogOut, Volume2, Plus, Edit2, Trash2, Menu, Users, MessageSquare, Headphones, Paperclip, Mic, MicOff, Shield, AlertTriangle, Sun, Moon } from 'lucide-react';
...
export default function Chat({ token, user: initialUser, onLogout, dark, onToggleTheme, serverUrl }: ChatProps) {
  ...
  const API_URL = serverUrl || (window.location.hostname === 'localhost' ? 'http://localhost:5000' : '');
  ...
  const sendFileMessage = (fileUrl: string, fileType: string, fileName: string) => {
    ...
    socket.emit('send-message', { channelId: activeChannelId, content: '', fileUrl, fileType, fileName });
  };
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => sendFileMessage(reader.result as string, file.type.startsWith('image/') ? 'image' : 'file', file.name);
  ...
  const toggleRecording = async () => {
    ...
    try {
      ...
      mr.onstop = () => {
        ...
        const reader = new FileReader();
        reader.onloadend = () => sendFileMessage(reader.result as string, 'audio', 'voice-message.webm');
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    socket.emit('send-message', { channelId: activeChannelId, content: input });
    setInput('');
  ...
  const handleDeleteChannel = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm('Purge this channel?')) return;
    try {
  ...
  return (
    <div className="flex flex-col h-[100dvh] overflow-hidden si-grid-bg" style={{ background: 'var(--bg-base)', color: 'var(--text-primary)' }}>
          ...
          <div className="flex-1 overflow-y-auto py-2 custom-scrollbar">
            ...
            <div className="mt-1">
              <div className="px-4 py-1.5 flex items-center justify-between">
                ...
                <button onClick={() => { setEditingChannel(null); setShowChannelModal(true); }}
                  className="transition-colors hover:opacity-80" style={{ color: 'var(--text-accent)' }}><Plus size={13} /></button>
              </div>
              {textChannels.map(ch => (
                  ...
                  return (
                    <div key={i} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      <div className="flex items-baseline gap-2 mb-1 px-1">
                        {!isMe && (
                          <div className="flex items-center gap-1.5 cursor-pointer hover:opacity-80" onClick={() => handleMessageUserClick(m.user?.username)}>
                            {m.user?.avatarUrl
                            ...
                            <span className="font-semibold text-xs transition-colors" style={{ color: 'var(--text-accent)' }}>{m.user?.username}</span>
                          </div>
                        )}
                      ...
                      {m.fileUrl ? (
                        ...
                        <div className="px-4 py-2.5 max-w-2xl leading-relaxed text-sm rounded-lg backdrop-blur-sm"
                          style={{
              ...
              <div className="px-5 h-5 z-10 flex items-center">
                {activeChannelId && typingUsers.has(activeChannelId) && typingUsers.get(activeChannelId)!.size > 0 && (
                  <div className="text-[10px] flex items-center gap-2" style={{ color: 'var(--text-muted)' }}>
                    ...
                    {Array.from(typingUsers.get(activeChannelId)!).join(', ')} is typing...
                  </div>
                )}
              </div>

              {/* Input */}
              <div className="p-4 z-10" style={{ borderTop: '1px solid var(--border)' }}>
                ...
                <form onSubmit={handleSend} className="flex items-center gap-2">
                  <button type="button" onClick={() => fileInputRef.current?.click()}
                    className="p-2 transition-colors hover:opacity-80 shrink-0" style={{ color: 'var(--text-muted)' }}>
                    <Paperclip size={16} />
                  </button>
      ...
      {showSettings && <SettingsModal token={token} user={user} onClose={() => setShowSettings(false)} onUpdateUser={(u) => { setUser(u); localStorage.setItem('stark_user', JSON.stringify(u)); }} dark={dark} />}
      {showChannelModal && <ChannelModal token={token} onClose={() => setShowChannelModal(false)} onSaved={() => { setShowChannelModal(false); fetchChannels(); }} editChannel={editingChannel} dark={dark} />}
    </div>
  );
}
